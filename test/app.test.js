...
var PersistedModel = Nodespeed.PersistedModel;
...
describe('app', function() {
  ...
  describe('app.model(Model)', function() {
    ...
    beforeEach(function() {
      ...
      db = Nodespeed.createDataSource({connector: Nodespeed.Memory});
  ...
  describe('app.model(ModelCtor, config)', function() {
    it('attaches the model to a datasource', function() {
      var previousModel = Nodespeed.registry.findModel('TestModel');
  ...
  describe('app.dataSource', function() {
    it('looks up the connector in `app.connectors`', function() {
      app.connector('custom', Nodespeed.Memory);
      app.dataSource('custom', {connector: 'custom'});
      expect(app.dataSources.custom.name).to.equal(Nodespeed.Memory.name);
    });
  ...
  describe.onServer('enableAuth', function() {
    ...
    it('auto-configures required models to provided dataSource', function() {
      ...
      var app = Nodespeed({localRegistry: true, loadBuiltinModels: true});
      require('./util/builtin-models')(app.registry);
  ...
  describe.onServer('app.get(\'/\', Nodespeed.status())', function() {
    it('should return the status of the application', function(done) {
      var app = Nodespeed();
      app.get('/', Nodespeed.status());
      request(app)
        .get('/')
        .expect(200)
  ...
  describe('app.connectors', function() {
    ...
    it('includes Remote connector', function() {
      expect(app.connectors.remote).to.equal(Nodespeed.Remote);
    });

    it('includes Memory connector', function() {
      expect(app.connectors.memory).to.equal(Nodespeed.Memory);
    });
  });
  ...
  describe('app.connector', function() {
    ...
    it('adds the connector to the registry', function() {
      app.connector('foo-bar', Nodespeed.Memory);
      expect(app.connectors['foo-bar']).to.equal(Nodespeed.Memory);
    });

    it('adds a classified alias', function() {
      app.connector('foo-bar', Nodespeed.Memory);
      expect(app.connectors.FooBar).to.equal(Nodespeed.Memory);
    });
  ...
  it('exposes Nodespeed as a property', function() {
    var app = Nodespeed();
    expect(app.loopback).to.equal(Nodespeed);
  });
